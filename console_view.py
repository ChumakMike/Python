...
def add_menu():
    ...
    show_days()
    days_number = list(map(int, input("\nEnter the number of days when to water: ").split()))
    days = [__convert(num) for num in days_number]
...
def add(name, time, days):
    print(controller.add(name, time, days))
